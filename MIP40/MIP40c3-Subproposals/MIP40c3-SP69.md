...
![image|690x755](upload://bheVyoiVxgz7Y1HO1jbo2pbqmTD.png)
...
**Travel and Conferences:** The team plans to travel once a quarter to collaborate more intensely, meet with other DAO members, and work our road map and strategic initiatives.

**Hardware & Welcome Package:** An allocation of $3,500 per new FT team member. (New members only, one-time cost).

**IT & Subscriptions:** We currently have subscriptions to Dune Analytics and various productivity applications (MS OneDrive, HackMD, etc.) and will test out various data providers to determine which product best suits our needs. We will also work with the Data Insights CU to see how our data needs can be served.
...
The Strategic Finance CU seeks to drive value to the protocol both through revenue growth and tangibly improving operational efficiency.

On the revenue side, our focus is on utilizing dormant protocol assets to generate income for the protocol. This work can be broken down into the following key components:
...
On the operational efficiency side, our focus is on delivering accurate and timely financial reports about the state of the system to token holders, as well as commercially relevant blockchain insights. This work can be broken down into a few key areas:
...
3) Facilitating Quarterly Strategic Reviews (QSRs) to review DAO-approved strategic focus areas, KPIs, and the DAOs progress against them.
...
In the event of rapid expansion outpacing our capacity, we would explore expanding the budget to accommodate an additional resource. However, our policy is to reach and exceed 100% capacity for a number of months with each of our Core Unit collaborators before looking into the need for adding an additional resource.

### Unwinding and Defunding

If SF-001 is defunded, all expansionary work will be halted to shift focus to wrapping up existing initiatives, creating documentation of existing processes, transitioning any processes, and archiving projects & data. Any funds not already allocated (such as employee compensation and software subscriptions) at the time of offboarding will be returned to the DAO through DSSBlow. All unvested MKR would be returned to the DAO as of the end-date. No one-time expenses are anticipated at this time.

### Considerations for Budget Modifications

The requested budget is a 43% increase over the first SF-001 budget. The increase is driven almost entirely by headcount compensation, which is driven by increased headcount. This is slightly offset by a lower contingency buffer (24% lower than the prior budget). Travel expenses have been increased nearly 3x, driven by a 69% (nice) increase in headcount and increasing the frequency of team meetings from twice a year to four times a year. The new travel budget remains lower than average.
...
- Create investment and risk framework for treasury asset allocations in collaboration with key DAO stakeholders and the community.
...
- Takeover all existing Financial Reporting, including but not limited to: monthly financial reports, expense reports, forecasting models, DCF models, Oracle gas models, etc.
...
- Finalize and launch a DAO financial and accounting close process.
...
Building relationships with top Universities will enable the DAO to better source talent. At this time, SF has sourced intern talent from Harvard Business School and UPenn (Wharton).

In addition to the above, we believe these individuals can add tremendous value to the DAO, even during the limited time they are with us on a full-time basis. If the trials are successful, we will endeavor to keep these individuals engaged on a part-time basis.

Hiring a Sr Financial Analyst will free up significant time for @adcv and myself, allowing us to work on higher value initiatives and projects. We currently spend 40+ hours per month on financial reporting, and related activities. With the higher bandwidth, we plan to invest significant time into:

1) Vault User Research
    a. Creating a NPS (Net Promoter Score) survey
...
![image|690x323](upload://tGJcBhT8izOG7fOZdBsbje4LJ2y.png)
