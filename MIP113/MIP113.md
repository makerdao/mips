...
---
* **4.6.3:** During the Pregame, any CVC Member can create a new CVC through a CVC creation message on the Maker Governance Forum based on the template of *4.6.3.1*. The new CVC must include a draft Governance Strategy Position Document in the CVC creation message. New CVCs are active from when they are created.
	* **4.6.3.1:**
...
* **4.7.1**: CVC Decisions are made via a signed message posted publicly to the Maker Governance Forum.
	...
	* **4.7.1.4**: The signed message must contain a clear record of the outcome, which must be congruent with the voting record.
...
* **4.7.3:** The available CVC Decisions are detailed in the subelements of this clause.
	* **4.7.3.1:** *Adding a CVC Member*. A simple majority of existing CVC Members can onboard a new CVC Member.
	* **4.7.3.2:** *Removing CVC Member*. A simple majority of existing CVC Members can remove a CVC Member.
	* **4.7.3.3:** *Communication infrastructure moderation rights management*. A simple majority of CVC Members can add or remove one or more moderators for one or more platforms.
	* **4.7.3.4:** *Scheduling Subcommittee Meeting*. A simple majority of CVC Members can schedule the time of a Subcommittee Meeting.
	* **4.7.3.5:** *Ratifying Position Paper*. A simple majority of CVC Members can ratify a Position Paper that has been publicly posted to the Maker Governance Forum.
	* **4.7.3.6:** *CD rejection*. A simple majority of CVC Members can reject a CD.
	* **4.7.5.7:** *CVC Split*. 25% of the CVC Member Voting power can initiate a CVC Split. A second vote then occurs where all CVC Members can choose to join the split. If the split gains more than 50% of all voting power, it will take over the name and CD followers.
