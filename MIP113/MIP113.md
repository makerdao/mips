...
At the end of the review period, the Governance Facilitators will post their response to the Atlas Intepretation request on the Maker Forum, along with the rationale for the decision. The post will also indicate which Facilitators voted for and against the majority decision.
...
Derecognition notices are issued publicly and justification and reasoning must be provided by the issuing Governance Facilitator. Derecognitions are determined based on the violation of the rules outlined in the code of conduct for each of the roles that an Alignment Conserver can assume.

#### 4.1.2

The Governance Facilitators must develop a code of conduct that specifically defines the behaviors prohibited for each of the Alignment Conservers: Aligned Voter Committee Members, Aligned Delegate, Facilitators and Budget Allocators. To carry out their task, they can propose meetings and/or surveys with each of these ecosystem actors to thoroughly understand which behaviors should be expressly prohibited.

#### 4.1.3

Derecognitions are recorded by adding the identity and known aliases or associated entities to 4.1.4A.
