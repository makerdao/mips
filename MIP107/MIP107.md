...
Core Protocol Expansion primarily concerns building new secure bridges between Maker Chain and other blockchains.
...
###### 5.2.3.3: A smart contract that regulates the amount of Dai funneled from the Surplus Buffer to Sagittarius Engine Farming, targeting 30% of the average accrued surplus over the last month.
---
##### 5.2.4: MKR redenominator
A system for wrapping (two-way conversion) of MKR into a new ERC-20 token of a different denomination, temporarily denoted RMKR. The Existing MKR token must remain forever, and RMKR can always be unwrapped back to MKR.

---
##### 5.2.5: SubDAO first wave
The first wave of SubDAOs include the deployment of 4 AllocatorDAOs with Allocator Vault Types, and 2 FacilitatorDAOs. 
###### 5.2.5.1: The first wave SubDAOs have no advanced internal tokenomics, but instead have RMKR farms that distribute to 3333 MKR per year to SubDAO token holders per SubDAO
###### 5.2.5.2: The first wave SubDAOs have active dssvest smart contracts that enable them to spend 3333 MKR per year as internal treasury. 
###### 5.2.5.3: If a first wave SubDAO holds more than 20 million Dai in assets for more than 1 month, the MKR dssvest specified in *5.2.5.2* must be turned off by Maker Governance until the SubDAO has converted assets in excess of 20 million Dai into Elixir. The actions for enforcing the rules of this element are taken through executive votes that must be triggered by the Protocol Engineering Facilitators.
###### 5.2.5.4: The first wave SubDAOs are distributed to Dai farms and the Sagittarius Engine. 70% of all SubDAO tokens are distributed to Dai Farms, and 30% of all SubDAO tokens are distributed to the Sagittarius Engine.
...
### 6: Governance Security engineering work
...
### 7: Multichain engineering work
Multichain engineering work includes developing Makers multichain strategy, Maker Teleport for Ethereum L2's, and the Ethereum Bridge for Maker Chain, setting the stage for long term Protocol Expansion work.

#### 7.1: Multichain engineering budget
The Multichain Engineering budget is specified in *7.1.1A*. It is implemented as a dssvest stream that is controlled by the Protocol Engineering Facilitators, and paid out to Ecosystem Actors in order to do work necessary for Multichain Engineering to function properly. The Multichain engineering budget should eventually be replaced by a permanent budget.
* **6.1.1A:**
...
### 8: Maker Chain
The Maker Chain is the last step of the Endgame Plan, that implements all of the advanced features and tokenomics of Endgame, including algorithmic SubDAO incubation and Elixir Wars.

It must be built with a modular blockchain architecture, have a VM suitable for AI driven code generation, and have critical features such as state rent and protocol level MEV capture, and support for native zero knowledge rollups.

It must be possible to recover from an emergency shutdown through a hard fork of the Maker Chain, including recovering collateral in bridges from other blockchains. To enable recovery of bridge assets in case of emergency shutdown, recovery multisigs controlled by resilient legal entities or independent, decentralized DAOs must be in place as backups that, with a high quorum, can access and transfer the collateral ONLY in the case that the Maker Protocol is shut down.

#### 8.1: Maker Chain bootstrapping budget
The Maker Chain bootstrapping budget is available to be used on work related to the Maker Chain project. The Maker Chain bootstrapping budget is specified in *8.1.1A*, and is allocated to relevant projects through a manual governance poll and executive vote using the weekly cycle, when triggered by the Protocol Engineering Facilitators posting a message to the Maker Core forum containing the relevant information. It is a one time budget that can only be increased or renewed through the regular CVC process. Some of the budget is immediate and some of it relative.
##### 8.1.0: *A relative budget means a budget that is only available if the Maker Protocol is generating enough income to actively burn, and the budget can then at most spend a smaller amount of what’s being burned, ensuring that the burn continues.*
##### 8.1.1A:
¤¤¤
